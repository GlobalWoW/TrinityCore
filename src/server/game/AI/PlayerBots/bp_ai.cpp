...
void PlayerbotAI::UpdateFollowActions(uint32 const diff)
{
    ...
    if (!fTarget->IsInWorld() || !fTarget->FindMap() || me->GetPhaseMask() != fTarget->GetPhaseMask() || (fTarget->GetTypeId() == TYPEID_PLAYER && fTarget->ToPlayer()->isInFlight()))
    {
        _followTimer = 5000;
        return;
    }
    ...
    if (me->GetMap() != fTarget->GetMap() || me->GetDistance2d(fTarget) > sWorld->GetMaxVisibleDistanceOnContinents())
    {
        Player* fPlayer = fTarget->ToPlayer();
        if (!fPlayer || !fPlayer->IsBeingTeleported())
...
bool PlayerbotAI::CastSpell(Unit* victim, uint32 spellId)
{
    return CastSpell(victim, sSpellMgr->GetSpellInfo(spellId));
}

bool PlayerbotAI::CastSpell(Unit* victim, SpellInfo const* spellInfo)
{
    ASSERT(victim);
    ...
    if (me->GetGlobalCooldownMgr().HasGlobalCooldown(spellInfo))
        return false;
    //power cost
    if (me->GetPower(Powers(spellInfo->PowerType)) < spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask()))
        return false;
    //range
...
bool PlayerbotAI::CastSpell(Item* item, uint32 spellId)
{
    return CastSpell(item, sSpellMgr->GetSpellInfo(spellId));
}

bool PlayerbotAI::CastSpell(Item* item, SpellInfo const* spellInfo)
{
    ASSERT(item);

    if (!spellInfo || me->HasSpellCooldown(spellInfo->Id) ||
        me->GetGlobalCooldownMgr().HasGlobalCooldown(spellInfo) ||
        me->GetPower(Powers(spellInfo->PowerType)) < spellInfo->CalcPowerCost(me, spellInfo->GetSchoolMask()))
        return false;
    //debug
...
inline bool PlayerbotAI::_CureTarget(Unit* target, SpellInfo const* spellInfo)
{
