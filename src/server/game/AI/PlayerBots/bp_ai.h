...
class PlayerbotAI
{
    public:
        ...
        //casts
        // units
        bool CastSpell(Unit* victim, uint32 spellId);
        bool CastSpell(Unit* victim, SpellInfo const* spellInfo);
        // items
        bool CastSpell(Item* item, uint32 spellId);
        bool CastSpell(Item* item, SpellInfo const* spellInfo);
        //  cures
        void CureGroup(Player* player, uint32 cureSpell, const uint32 diff);
    private:
        inline bool _CureTarget(Unit* target, SpellInfo const* spellInfo);
