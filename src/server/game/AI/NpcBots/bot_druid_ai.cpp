...
public:
    ...
    struct bot_druid_ai : public bot_minion_ai
    {
        ...
        void warstomp(uint32 const diff)
        {
        ...
        void removeFeralForm(bool force = false, bool init = true, uint32 const diff = 0)
        {
        ...
        void doBearActions(uint32 const diff)
        {
        ...
        void doCatActions(uint32 const diff)
        {
            if (GetHealthPCT(me) < 75)
                HealTarget(me, GetHealthPCT(me), diff);
            opponent = me->getVictim();
        ...
        void doBalanceActions(uint32 const diff)
        {
        ...
        bool MassGroupHeal(Player* gPlayer, uint32 const diff)
        {
        ...
        bool HealTarget(Unit* target, uint8 hp, uint32 const diff)
        {
        ...
        bool BuffTarget(Unit* target, uint32 const diff)
        {
        ...
        void CheckRoots(uint32 const diff)
        {
        ...
        void CheckBattleRez(uint32 const diff)
        {
        ...
        void ReduceCD(uint32 const diff)
        {
            CommonTimers(diff);
