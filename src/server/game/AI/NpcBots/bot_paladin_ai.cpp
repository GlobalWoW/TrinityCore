...
public:
    ...
    struct paladin_botAI : public bot_minion_ai
    {
        ...
        void HOFGroup(Player* pTarget, uint32 const diff)
        {
        ...
        bool HOFTarget(Unit* target, uint32 const diff)
        {
       ...
       /*else */if (spellInfo->GetAllEffectsMechanicMask() & (1<<MECHANIC_SNARE) || 
        ...
        void HOSGroup(Player* hTarget, uint32 const diff)
        {
        ...
        bool HOSTarget(Unit* target, uint32 const diff)
        {
            if (!target || target->isDead()) return false;
        ...
        bool HS(Unit* target, uint32 const diff)
        {
            if (!target || target->isDead()) return false;
        ...
        void Repentance(uint32 const diff, Unit* target = NULL)
        {
        ...
        void TurnEvil(uint32 const diff)
        {
