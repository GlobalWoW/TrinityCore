...
void bot_minion_ai::BuffAndHealGroup(Player* gPlayer, uint32 const diff)
{
...
void bot_minion_ai::CureGroup(Player* pTarget, uint32 cureSpell, uint32 const diff)
{
...
bool bot_minion_ai::CureTarget(Unit* target, uint32 cureSpell, uint32 const diff)
{
    return CanCureTarget(target, cureSpell, diff) ? doCast(target, cureSpell) : false;
}
// determines if unit has something to cure
bool bot_minion_ai::CanCureTarget(Unit* target, uint32 cureSpell, uint32 const diff) const
{
...
void bot_minion_ai::UpdateRations() const
{
    ...
    if (me->getPowerType() == POWER_MANA && !me->IsMounted() && !me->isMoving() && !CCed(me) && 
        !me->isInCombat() && !IsCasting() && rand()%100 < 20 && GetManaPCT(me) < 80 && 
        !me->HasAura(DRINK))
    {
    ...
    if (!me->IsMounted() && !me->isMoving() && !CCed(me) && 
        !me->isInCombat() && !IsCasting() && rand()%100 < 20 && GetHealthPCT(me) < 80 && 
        !me->HasAura(EAT))
    {
        me->CastSpell(me, EAT);
        me->SetStandState(UNIT_STAND_STATE_SIT);
...
//Including health cals, set and regeneration
...
void bot_minion_ai::BreakCC(uint32 const diff)
{
